-- Minimal TDS Loadout Library Bundle
local modules = {}
...
function InventoryModule.equipLoadout(targetTowers)
    ...
    -- Filter target towers by ownership. If not owned, skip it.
...
local function loadModule(name)
    local content = modules[name]
    if not content then error("Module not found: " .. name) end
